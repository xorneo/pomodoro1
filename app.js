...
// Visibility change handler (adjusts duration if tab goes inactive)
let lastActiveTime = null;
function handleVisibilityChange() {
  if (document.hidden) {
    lastActiveTime = Date.now();
  } else {
    if (lastActiveTime && timerState === 'running') {
      const elapsedSeconds = Math.round((Date.now() - lastActiveTime) / 1000);
